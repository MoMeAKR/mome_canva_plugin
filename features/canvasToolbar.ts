...
// 1. UPDATE INTERFACE: Allow 'callback' OR 'command'
export interface ToolbarButton {
    icon: string;
    tooltip: string;
    command?: string;      // Optional: keep for backward compatibility
    callback?: () => void; // Optional: The new direct function approach
}
...
export class CanvasToolbar {
    ...
    private createButton(config: ToolbarButton): HTMLElement {
        ...
        // 2. UPDATE CLICK HANDLER: Check for callback first
        button.onclick = (e) => {
            e.preventDefault(); 
     ...
     */
    private executeCommand(commandId: string) {
        // Handle full ID or partial ID
        const app = this.plugin.app as any;
        
        // Try exact match first
        if (app.commands.executeCommandById(commandId)) return;

        // Try prepending plugin ID if not found (helper)
